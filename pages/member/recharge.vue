<template>
	<view class="pd10_15">
		<view class="pd10_15 ft14" @tap="showinfo()">{{$t('score.buytip')}}</view>
		<view class="flex wrap space ">
			<view v-for="(item,index) in productList" :key="index" :class="selid==item.id?'selected':''" class="box pd20_15 item" 
			@tap="changeNum(index)">
				<view class="ft14 num ftw600">{{item.name}}</view>
...
<script>
	export default{
		...
		methods:{
			...
			async getproduct(){
				let data = {};
				let [err,res] =await this.$httpas.get('/api/user/score_group',data,{token:true});
				if(res.data.code==1){
...
<style>
	.item{width: 300rpx; text-align: center;margin: 20rpx;border: solid 1px #eeeeee;}
	.selected{
